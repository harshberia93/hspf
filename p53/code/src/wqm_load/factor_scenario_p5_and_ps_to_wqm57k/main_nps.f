...
********** ps, atdep, septic variables
      ...
      integer lenmodscen    ! the scenario points to a directory in
                            ! ./input/sceanrio/wqm/RiverFactors/

************* river factor for scenarios that are a percentage of other
***********  scenarios, useful for running geographic isolation runs
      real LRsegRiverFactor(maxrsegs,0:maxBvar) ! extra column for scratch
      real RsegRiverFactor(maxrsegs,0:maxBvar)! extra column for scratch
************** 
      character*13 LRsegRsegs(maxrsegs)  ! rseg portion of lrsegs
...
******** get River Factor for each lrseg
      do nlrseg = 1,nlrsegs
        LRsegRsegs(nlrseg) = lrseg(nlrseg)(7:19)
      end do
      call getRiverFactor(
     I                    modscen,LRsegRsegs,nlrsegs,nBvar,Bname,
     O                    LRsegRiverFactor)  ! indexed to nlrsegs
      do nlrseg = 1,nlrsegs
        do nq = 1,nBvar      ! check negative or missing
          if (LRsegRiverFactor(nlrseg,nq).lt.-0.01) go to 997
        end do
      end do
...
******** get River Factor for each rseg
      do nrseg = 1,nrsegs
        print*,nrseg,' ',rsegs(nrseg)
      end do
      print*,'nBvar ',nBvar
      call getRiverFactor(
     I                    modscen,rsegs,nrsegs,nBvar,Bname,
     O                    RsegRiverFactor)  ! indexed to nrsegs
      do nrseg = 1,nrsegs
        do nq = 1,nBvar      ! check negative or missing
          if (RsegRiverFactor(nrseg,nq).lt.-0.01) go to 9971
        end do
      end do
...
997   report(1) = ' Negative value for river factor not allowed'
      report(2) = ' segment '//LRsegRsegs(nlrseg)//
     .            ' bay variable '//Bname(nq)
      write(report(3),*) ' river factor = ',LRsegRiverFactor(nlrseg,nq)
      go to 999

9971  report(1) = ' Negative value for river factor not allowed'
      report(2) = ' segment '//rsegs(nrseg)//
     .            ' bay variable '//Bname(nq)
      write(report(3),*) ' river factor = ',RsegRiverFactor(nrseg,nq)
      go to 999
