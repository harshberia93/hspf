...
*************  END DECLARATIONS  ***************************************
      if (year1.gt.CMAQyear1 .or. year2.lt.CMAQyear2) go to 994
...
**************** DETREND WET LOADS
     ...
     .                 * (1.0 + real(scenyear-year)*no3slope)
        wetnh3load(nd) = wetnh3load(nd)
     .                 * (1.0 + real(scenyear-year)*nh3slope)
        call tomorrow(year,month,day)
      end do
...
994   report(1) = 'years requested must span the CMAQ years'
      write(report(2),*) 'years requested ',year1,'-',year2
      write(report(3),*) 'CMAQ years ',CMAQyear1,'-',CMAQyear2
      go to 999
