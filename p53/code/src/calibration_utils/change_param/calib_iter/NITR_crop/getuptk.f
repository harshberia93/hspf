...
C        if (clu .eq. 'alf') then
C          if (outline(13:29) .eq. 'NITROGEN FIXATION')  then
C            read(dfile,'(a140)',err=992,end=111) outline
C            read(dfile,'(a140)',err=992,end=111) outline
C            read(outline(123:131),*) Nfix(ny)
C            if (thislseg(:6) .eq. 'A24023')
C     .        print*, 'after Nfix', Nfix(ny)
C          end if
C          TNuptk(ny) = NH4uptk(ny)+ NO3uptk(ny)+ Nfix(ny)
C        end if
...
111   close(dfile)
      ...
      TNSum = 0.
      do i = 2, years
       if (thislseg(:6) .eq. 'A24023') print*, i, ' ',TNuptk(i) 
       TNSum = TNSum + TNuptk(i)
      end do

      simuptk = TNSum/real(years-1)
      if (thislseg(:6) .eq. 'A24023') print*, simuptk
      return
