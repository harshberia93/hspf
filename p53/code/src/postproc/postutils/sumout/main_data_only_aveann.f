...
*********** open all data files to feed scenario builder postprocessor
     ...
     .     '/AllData_'//basin(:lenbasin)//'_'//cy1//'_'//cy2//'_'//
...
************ write output
      if (doEOF) then
        fnam = sumoutdir//'aveann/'//rscen(:lenrscen)//
     .       '/'//basin(:lenbasin)//'_'//cy1//'_'//cy2//'_data_EOF.csv'
        print*,fnam(:78)
     ...
     .                        (',',allEOFatdep(nload),nload=1,nloads)
      ...
      if (doEOS) then
        fnam = sumoutdir//'aveann/'//rscen(:lenrscen)//
     .       '/'//basin(:lenbasin)//'_'//cy1//'_'//cy2//'_data_EOS.csv'
        print*,fnam(:78)
        open(11,file=fnam,status='unknown',iostat=err)
        if (err.ne.0) go to 991
     ...
     .                        (',',allEOSsep(nload),nload=1,nloads)
        write(11,1234,err=951) 'atdep',
     .                        (',',allEOSatdep(nload),nload=1,nloads)
      ...
      if (doDEL) then
        fnam = sumoutdir//'aveann/'//rscen(:lenrscen)//
     .       '/'//basin(:lenbasin)//'_'//cy1//'_'//cy2//'_data_DEL_'//
     .       DFmethod//'.csv'
        print*,fnam(:78)
        open(11,file=fnam,status='unknown',iostat=err)
        if (err.ne.0) go to 991
     ...
     .                        (',',allDELsep(nload),nload=1,nloads)
        write(11,1234,err=951) 'atdep',
     .                        (',',allDELatdep(nload),nload=1,nloads)
