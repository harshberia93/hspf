...
******** user specifies which one to use
     ...
     .                     .and.DFmethod.ne.'res') go to 992
      ...
      call sumout(rscen,basin,year1,year2,iEOF,iEOS,iDEL,DFmethod)
...
************************************************************************
      subroutine sumout(rscen,basin,year1,year2,iEOF,iEOS,iDEL,DFmethod)
      implicit none
      include '../../../lib/inc/standard.inc'
...
********* reading variables for loads
      ...
      real EOFps(nloadmax,year1:year2,nmonths) 
      ...
      real EOSps(nloadmax,year1:year2,nmonths) 
      ...
      real DELps(nloadmax,year1:year2,nmonths) 
...
Clrseg      real allEOFlrseg(maxrsegs*maxL2R,nloadmax+1,year1:year2,nmonths) 
      ...
      real allEOFps(nloadmax+1,year1:year2,nmonths) 
...
Clrseg      real allEOSlrseg(maxrsegs*maxL2R,nloadmax+1,year1:year2,nmonths) 
      ...
      real allEOSps(nloadmax+1,year1:year2,nmonths) 
...
Clrseg      real allDELlrseg(maxrsegs*maxL2R,nloadmax+1,year1:year2,nmonths) 
      ...
      real allDELps(nloadmax+1,year1:year2,nmonths) 
...
Clrseg     .                                + EOFatdep(nload,ny,nm)
                  allEOFps(nload,ny,nm) = allEOFps(nload,ny,nm) 
     .                                  + EOFps(nload,ny,nm)
...
Clrseg     .                                + EOSatdep(nload,ny,nm)
                  allEOSps(nload,ny,nm) = allEOSps(nload,ny,nm)
     .                                  + EOSps(nload,ny,nm)
...
Clrseg     .                                + DELatdep(nload,ny,nm)
                  allDELps(nload,ny,nm) = allDELps(nload,ny,nm)
     .                                  + DELps(nload,ny,nm)
...
************* point source
     ...
     .        'pointsourc.csv'
        ...
        do ny = year1,year2
          do nm = 1,nmonths
            write(11,1234,err=951) ny,',',nm,
     .                     (',',allEOFps(nload,ny,nm),nload=1,nloads)
          end do
        end do
        close(11)
...
************* point source
     ...
     .        'pointsourc.csv'
        ...
        do ny = year1,year2
          do nm = 1,nmonths
            write(11,1234,err=951) ny,',',nm,
     .                     (',',allEOSps(nload,ny,nm),nload=1,nloads)
          end do
        end do
        close(11)
...
************* point source
     ...
     .        'pointsourc_'//DFmethod//'.csv'
        ...
        do ny = year1,year2
          do nm = 1,nmonths
            write(11,1234,err=951) ny,',',nm,
     .                     (',',allDELps(nload,ny,nm),nload=1,nloads)
          end do
        end do
        close(11)
...
Crseg     .          //basin(:lenbasin)//'_'//cy1//'_'//cy2//'_'//
Crseg     .          rsegs(nrseg)//'_'//DFmethod//'.csv'
Crseg          print*,fnam(:78)
Crseg          open(11,file=fnam,status='unknown',iostat=err)
Crseg          if (err.ne.0) go to 991
Crseg          write(11,'(a)',err=951) header
...
Clseg     .          //basin(:lenbasin)//'_'//cy1//'_'//cy2//'_'//
Clseg     .          lsegs(nlseg)//'_'//DFmethod//'.csv'
Clseg          print*,fnam(:78)
Clseg          open(11,file=fnam,status='unknown',iostat=err)
Clseg          if (err.ne.0) go to 991
Clseg          write(11,'(a)',err=951) header
...
Clrseg     .          //basin(:lenbasin)//'_'//cy1//'_'//cy2//'_'//
Clrseg     .          lrrsegs(nlrseg)//'-'//lrlsegs(nlrseg)//'_'//DFmethod//'.csv'
Clrseg          print*,fnam(:78)
Clrseg          open(11,file=fnam,status='unknown',iostat=err)
Clrseg          if (err.ne.0) go to 991
Clrseg          write(11,'(a)',err=951) header
