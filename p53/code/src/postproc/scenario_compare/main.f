...
*********** variables to write lock file and wait until scenario
*********** compare file is not being used
      character*200 lock  ! name of lock file to prevent stale data
      integer lfile
      logical lockfileexist
      integer locktry, maxlocktries  ! number of attempts at opening
      parameter (maxlocktries = 40) ! shouldn't take long
...
*********** write output
************** loop over load types.  If the file previously exists
********** read and store it, then write new file with old and new segs
*********  If no file exists, start one
     ...
     .         rscen(:lenrscen)//'_'//'scenario_compare_'//
     .         loadname(nl)//'.csv'

************** start lock
        call findopen(lfile)
        lock = pardir//'river/'//paramscen(:lenparamscen)//'/'//
     .         rscen(:lenrscen)//'_'//loadname(nl)//'.lock'
        do locktry = 1,maxlocktries
          inquire(file=lock,exist=lockfileexist)
          if (lockfileexist) then
            call sleep(1)
            print*,'wait for file access ',locktry,' of ',maxlocktries
            print*,fnam
          else
...
********** file locked
        ...
        inquire (file=fnam,exist=fileexists)

        nrsegs = 0

        if (fileexists) then  ! file previously exists, read first
          open(outfil,file=fnam,status='old',iostat=err)
          if (err.ne.0) go to 991
          read(outfil,*) dummy  ! header line
          do 
            nrsegs = nrsegs + 1
            read(outfil,*,err=992,end=111) rsegs(nrsegs),
     .                                dummy,ts(nrsegs),sstr(nrsegs),
     .                                sp(nrsegs),ss(nrsegs),sa(nrsegs),
     .                                dummy,tc(nrsegs),cstr(nrsegs),
     .                                cp(nrsegs),cs(nrsegs),ca(nrsegs)
            if (rsegs(nrsegs).eq.rseg) nrsegs = nrsegs - 1
          end do
111       close(outfil)
          nrsegs = nrsegs - 1
        end if

*********** open and write new file, including any previous segs
        open(outfil,file=fnam,status='unknown',iostat=err)
        if (err.ne.0) go to 991

        write(outfil,*,err=951) 'seg,scen,Total,Stream,PS,Sep,Atdep,',
     .                  'calscen,Total,Stream,PS,Sep,Atdep'
        do ns = 1,nrsegs
          write(outfil,1234,err=951)rsegs(ns),
     .                      rscen,ts(ns),sstr(ns),sp(ns),ss(ns),sa(ns),
     .                      calscen,tc(ns),cstr(ns),cp(ns),cs(ns),ca(ns)
        end do
        write(outfil,1234,err=951) rseg,
     .                     rscen,totalScen(nl),
...
9911  report(1) = 'Error opening file:'
      report(2) = lock
      report(3) = ' '
      go to 999
