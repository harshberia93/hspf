...
****************  daily load
      real dEOFrvar(ndaymax,maxRvar) ! daily by river var
      real dEOSrvar(ndaymax,maxRvar) ! daily by river var
      real dEOFload(ndaymax,nloadmax)     ! daily by load var
...
************************************************************************
                call gethourdsn(wdmlnd,sdate,edate,   ! 1
     I                         Ldsn(Rvar,l,Lvar),
     O                         nvals,hval)                           
...
******************** factor, if so, get the factor
     ...
     .                          * bmpfac(jday+day1-1,nb,l)
                    do hour = 1,24
                      ...
                      dEOSrvar(jday,Rvar) = dEOSrvar(jday,Rvar)+hourload
...
************ calculate annual loads by BMP constituent
                ...
                do Rvar = 1,nRvar
                  ...
                  day = reqSRTd
                  do jday = 1,jdaytot
                    annEOSbcon(year) = annEOSbcon(year) 
     .                               + dEOSrvar(jday,Rvar)
                    call tomorrow(year,month,day)
                  end do
                end do
...
****************** end QUICK FIX
     ...
     .                   (annEOSbcon(year) - AnnualPounds(year,nb,l,ns))
