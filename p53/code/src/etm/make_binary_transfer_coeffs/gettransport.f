...
**************END DECLARATION ******************************************
      ...
      fnam = pardir//'transport/'//tranfile(:lastran)
      ...
      ntranps = 0
      foundBOD = .false.
      do kl = 1,nbcon*nlu       ! find the order of the bmps
        order(kl,1) = 0
        order(kl,2) = 0
      ...
      do while (ifound.lt.numsegs)  ! file until all land segs are found
        ...
        if (Trseg.eq.rseg) then
          ...
          do kl = 1,ntranps                            ! now that we have the land segment, read across
            ...
            call fread(dline,x)
            trandata(nl2r,order(kl,1),order(kl,2)) = x
          end do
        end if
      end do
      ...
      if (.not.foundBOD) then
        ...
        if (found) then   ! inherit N or P or average
          ...
          if (foundTN.and.foundTP) then ! take average
            do i = 1, maxL2R           
              do l = 1,nlu
                trandata(i,nBOD,l) = (trandata(i,nTN,l)
     .                             +  trandata(i,nTP,l)) / 2.0
              end do
            end do
            ...
            do i = 1, maxL2R             
              do l = 1,nlu
                trandata(i,nBOD,l) = trandata(i,nTP,l)
              end do
            end do
          else
            go to 989
