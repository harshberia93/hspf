...
************************************************************************
**  main program.  loop over all lines and populate variables for     **
**   each type of output. write each type of output separately        **
**   write an echo text file                                          **
************************************************************************
** there is one data mismatch in this procedure.  They only measure   **
**  dissolved PO4 and TP.  Particulate inorganic phosphorus ends up   **
**  as refractory organic since it is found by subtraction.           **
************************************************************************
** For phase 5.3, read 3 separate files                               **
**   CSO file by day and month                                        **
**   nonsig and sig files daily load by month                         **
**  All three are by lrseg, so no need for linkage table              **
**  this was used for the river calibration of phase 5.3 only         **
**  Need to update this to make sure that estuarine model is ready    **
**   to go.                                                           **
************************************************************************
      implicit none
      ...
      character*40 dataversion,nonCSOfnam,CSOfnam ! location of raw data
      integer lenpsscen,lendv,lennonCSOfnam,lenCSOfnam

      integer y1,y2  ! start and end year
      parameter (y1=1984, y2=2005)
      integer jday
...
***************** the primary key for loads is a combined key of 
********** NPDES number, discharge number, and county
C      integer maxfacs,nfacs,nf
C      parameter (maxfacs = 4000)
C      character*10 facility(maxfacs),Tfac
C      integer DischargePoint(maxfacs),TdisPoint
C      integer CountyFips(maxfacs),Tfips
C      character*6 fachydcell(maxfacs),facw57cell(maxfacs)
C      character*6                     facw13cell(maxfacs)
C      character*13 facRseg(maxfacs)
C      character*6 facLseg(maxfacs)
      character*10 Tfac
      integer TdisPoint
      integer Tfips
...
C      character*6 hydcell(maxcells)
C      character*6 w13cell(maxcells)
      integer n57cells  ! total # of cells
      integer n57cell   ! index of current cell
C      integer n13cells  ! total # of cells
C      integer n13cell   ! index of current cell
      integer lencell
...
C      real flow13(maxcells,y1:y2,12)
C      real heat13(maxcells,y1:y2,12)
C      real bodx13(maxcells,y1:y2,12)
C      real tssx13(maxcells,y1:y2,12)
C      real nh3x13(maxcells,y1:y2,12)
C      real no3x13(maxcells,y1:y2,12)
C      real ornx13(maxcells,y1:y2,12)
C      real po4x13(maxcells,y1:y2,12)
C      real orpx13(maxcells,y1:y2,12)
C      real doxx13(maxcells,y1:y2,12)
C      real orcx13(maxcells,y1:y2,12)

*************** variables for CSOs for LRsegs
      integer maxCSOlrsegs,nCSOlrsegs,nCSOlrseg ! CSO-lrseg combination
...
************** utility variables
      character*300 longline,longfnam,command
      character*2 state

      integer ny,nm,nd,nday,nd2
...
**************** END DECLARATIONS **************************************
******** get input data
      read*, psscen, dataversion, nonCSOfnam, CSOfnam
...
C        do ny = y1,y2
C          do nm = 1,12
C            flow13(ns,ny,nm) = 0.0
C            heat13(ns,ny,nm) = 0.0
C            bodx13(ns,ny,nm) = 0.0
C            tssx13(ns,ny,nm) = 0.0
C            nh3x13(ns,ny,nm) = 0.0
C            no3x13(ns,ny,nm) = 0.0
C            ornx13(ns,ny,nm) = 0.0
C            po4x13(ns,ny,nm) = 0.0
C            orpx13(ns,ny,nm) = 0.0
C            doxx13(ns,ny,nm) = 0.0
C            orcx13(ns,ny,nm) = 0.0
C          end do
C        end do
C      end do
...
C**************** find nf (facility number)
C         found = .false.
C         do nf = 1,nfacs
C           if (Tfac.eq.facility(nf)) then
C             if (TdisPoint.eq.DischargePoint(nf)) then
C               if (Tfips.eq.CountyFips(nf)) then
C                 found = .true.
C                 exit
C               end if
C             end if
...
C**************** get total number of wqm13 cells represented
C        found = .false.
C        do n13cell = 1,n13cells
C          if (w13cell(n13cell).eq.facw13cell(nf)) then
C            found = .true.
C            exit
C          end if
C        end do
C        if (.not.found) then
C          n13cells = n13cells + 1
C          if (n13cells.gt.maxcells) go to 9932
C          n13cell = n13cells
C          w13cell(n13cell) = facw13cell(nf)
C        end if
...
C************** add this line to the 13k bay model variables
C        flow13(n13cell,ny,nm) = flow13(n13cell,ny,nm) + Tflow*mg2acft
C        heat13(n13cell,ny,nm) = heat13(n13cell,ny,nm) 
C     .                         + Tflow*mg2acft*tempC(nm)*acftC2heat
C        doxx13(n13cell,ny,nm) = doxx13(n13cell,ny,nm) + Tdox
C        nh3x13(n13cell,ny,nm) = nh3x13(n13cell,ny,nm) + Tnh3
C        no3x13(n13cell,ny,nm) = no3x13(n13cell,ny,nm) + Tno3
C        po4x13(n13cell,ny,nm) = po4x13(n13cell,ny,nm) + Tpo4
C
C        Tbod = min(Torn/bod2lorn,Torp/bod2lorp,Tbod)
C        ornx13(n13cell,ny,nm) = ornx13(n13cell,ny,nm) 
C     .                         + Torn-Tbod*bod2lorn
C        orpx13(n13cell,ny,nm) = orpx13(n13cell,ny,nm) 
C     .                         + Torp-Tbod*bod2lorp
C        orcx13(n13cell,ny,nm) = orcx13(n13cell,ny,nm) 
C     .                         + (Torn-Tbod*bod2lorn) * rorn2rorc
C        bodx13(n13cell,ny,nm) = bodx13(n13cell,ny,nm) + Tbod
C        tssx13(n13cell,ny,nm) = tssx13(n13cell,ny,nm) 
...
************ write out list of LRsegs to make sure that GEO file
*********** is updated
     ...
     .           '/lrsegs.csv'
      ...
      print*,longfnam
      do nlrseg = 1,nlrsegs
        write(dfile,*)'point source lrseg ,',lrseg(nlrseg)
      end do
      do nCSOlrseg = 1,nCSOlrsegs
        write(dfile,*)'CSO lrseg ,',CSOlrseg(nCSOlrseg)
      end do
      close(dfile)
...
C********** echo out lrseg point sources
...
C     .                        lrflow(nlrseg,ny,nm)*real(nday),',',
C     .                        lrheat(nlrseg,ny,nm)*real(nday),',',
C     .                        lrbodx(nlrseg,ny,nm)*real(nday),',',
C     .                        lrdoxx(nlrseg,ny,nm)*real(nday),',',
C     .                        lrnh3x(nlrseg,ny,nm)*real(nday),',',
C     .                        lrno3x(nlrseg,ny,nm)*real(nday),',',
C     .                        lrpo4x(nlrseg,ny,nm)*real(nday),',',
C     .                        lrtssx(nlrseg,ny,nm)*real(nday),',',
C     .                        lrornx(nlrseg,ny,nm)*real(nday),',',
C     .                        lrorpx(nlrseg,ny,nm)*real(nday),',',
C     .                        lrorcx(nlrseg,ny,nm)*real(nday)
C          end do
C        end do
C      end do
C      close(dfile)
1234  format(a6,a1,a13,a1,i4,a1,i2,11(a1,e10.3))
...
C988   report(1) = 'hydro cell and wqm cell do not have 1:1 match in'
C      report(2) = 'facility file'
C      report(3) = hydcell(n57cell)//' '//w57cell(n57cell)
C      go to 999

989   write(report(1),*) 'facility ',Tfac,' Discharge Point ',TdisPoint,
     .                   ' and FIPS ',Tfips
      report(2) = ' were found in the data file, but not in the'
      report(3) = ' link file'
      go to 999
...
9932  report(1) = 'more 13k cells than anticipated'
      report(2) = 'increase maxcells in ./pp/src/'
      report(3) = 'calibration_utils/wdm/src/make_point_source_wdms'
      go to 999
...
998   report(1) = 'TN mass balance problem in file:  on line:'
      report(2) = fnam
      report(3) = longline 
      go to 999
