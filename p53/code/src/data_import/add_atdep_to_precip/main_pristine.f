...
************************************************************************
      ...
      real no3conc  ! constant concentration in rainfall
      parameter (no3conc = 0.035)  ! in mg/l
      real dryno3annual  ! lbs/ac/year
      parameter (dryno3annual=0.65)
...
********** if no conc on that day, search +- 1 month for close rain
      ...
      do nd = 1,ndays
        wetno3load(nd) = dailyrain(nd) * no3conc * mglin2lb
      end do
...
************* populate dry variables
      do nd = 1,ndays
        dryno3load(nd) = dryno3annual / 365.25
        drynh3load(nd) = 0.0
      end do
