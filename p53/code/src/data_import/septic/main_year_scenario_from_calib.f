...
************************************************************************
      ...
      integer ndays,year,nb
      real factor,denom

      integer staticj,staticyear
...
**************** END DECLARATIONS **************************************
      read*,sdate(1),edate(1),dfnam,sepscen,staticyear
...
******** housekeeping
      ...
      staticj = julian(sdate(1),1,1,staticyear,7,1)
...
************* sort in case the data file was out of order
        ...
        indx2 = 2
        do nb = 1,nbreaks-1
          if (staticj.ge.BJday(nb)) indx2 = nb + 1
        end do
        indx1 = indx2-1
...
********* interpolate 
        ...
        value = septic(indx1) + factor*real(staticj-BJday(indx1))
        if (value.lt. -0.1) go to 996
