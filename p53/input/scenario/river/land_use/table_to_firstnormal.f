...
***************** end of declarations **********************************
      print*, 'enter name of file to verticalize'
      read*,fnam
      newfnam = 'vert_'//fnam
...
C        write(cy,'(i4)') year
C        cy = 'full'
C        fnam = 'land_use_p52_full.csv'
        open(11,file=fnam,status='old')
        print*,fnam

        write(12,*) 'rseg,lseg,landuse,Acres' ! header
...
************ loop over lines and write all to new file
        do 
          ...
          do nf = 1,nfields
            write(12,*) rseg,',',lseg,',',clu(nf),',',acres(nf)
          end do
        end do
111     close(11)
C      end do  ! end loop over years
