...
###############  the input years are only used for average annual output
  ...
  if (${#argv} != 5) then
    if (${#argv} != 4) then
      echo ' '
      echo 'usage:  run_etm_and_land_and_dat_simultaneously.csh scenario basin aveyear1 aveyear2'
      echo ' or     run_etm_and_land_and_dat_simultaneously.csh scenario basin aveyear1 aveyear2 tree'
      echo ' '
      exit
    endif
  endif
...
######## MAKE BINARY TRANSFER COEFFICIENT FILE
    echo $scenario, $seg  `${tree}/run/standard/random.sh` | $tree/code/bin/make_binary_transfer_coeffs.exe
      if (-e problem) then
        echo ' '
        cat problem
        exit
