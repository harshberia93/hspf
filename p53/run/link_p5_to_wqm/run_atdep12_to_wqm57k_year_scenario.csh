#!/bin/csh
  ...
  if (${#argv} != 1) then
    echo ' '
    echo ' running this script creates atmospheric deposition inputs'
    echo '  for a scenario representative of a given year within the '
    echo '  calibration data set'
    ...
    echo ' '
    echo ' To make this script run, type: run_atdep12_to_wqm57k_year_scenario.csh GO'
    echo ' '
    exit
  endif
  if ($argv[1] != 'GO') then
...
###### linkdirectory contains the wqm to cmaq linkage
  set code = $tree/code/bin/atdep12_to_wqm57k_year_scenario.exe
  set year1 = 1985
  set year2 = 2005
  set ScenYear = 2002
  set atdepscen = year2002
...
####### run the code
  ...
  echo $atdepscen $linkdirectory $ScenYear $CMAQbase $CbaseDir2 $CbaseSuperLong | $code
...
######### self-documentation
  ...
  echo ' the data are detrended to the year '$ScenYear >> $notefile
